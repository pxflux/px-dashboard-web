...
export default {
  ...
  computed: {
    ...
    published() {
      return this.accountPlace && this.accountPlace.published
        ? this.accountPlace.published
        : false;
    },
    image() {
      return this.accountPlace && this.accountPlace.image
        ? this.accountPlace.image
        : {
            displayUrl: null,
            storageUri: null
          };
    ...
    init() {
      if (!this.isNew && this.accountId) {
        this.setRef({
          ...
          ref: this.$firebase
            .database()
            .ref("accounts/" + this.accountId + "/places/" + this.placeId)
        });
      }
    },
